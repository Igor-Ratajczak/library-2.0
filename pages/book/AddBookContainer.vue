<template>
  <div id="add_book_container" class="window">
    ...
    <div id="list_book">
      <div
        v-for="item in books"
        :key="item.id"
        :class="{ is_in_idb: isInDB(item.id) }"
        class="book"
        @click.once="($event: Event) => setClass($event, item)"
      >
        <div class="title">
          <b class="text-1sxl">{{ item.volumeInfo.title }}</b>
          <p v-if="item.volumeInfo.subtitle" class="text-1xl">
            {{ item.volumeInfo.subtitle }}
          </p>
        </div>
        <div class="authors">
          <h3 v-for="author in item.volumeInfo.authors" :key="author">
            {{ author }}
          </h3>
        </div>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const setClass = (event: Event, item: BookItem) => {
        ...
        if (target) {
          if (!target.classList.contains('is_in_idb')) {
            const book: Book = {
              book_id: item.id || '',
  ...
  div#add_book_container {
    ...
    #list_book {
      ...
      .book {
        ...
        height: 20em;
        display: grid;
        grid-template: auto 10% 80% / 50% 50%;
        position: relative;
        cursor: pointer;
        ...
        &:not(.is_in_idb):hover::after {
          opacity: 1;
        }

        &.animation:not(.is_in_idb)::after {
          opacity: 1;
          animation: is_in_idb 0.5s forwards;
        }
